...
', $db_host, $db_user, $db_passwd, $db_port);
  ...
  function generate_dump() {
    ...
    $cmd = sprintf("mysqldump --defaults-file=/dev/fd/3 %s --single-transaction --skip-quick --extended-insert --insert-ignore --max-allowed-packet=512M --no-autocommit --skip-add-locks %s", $gtid_option, escapeshellcmd(drush_get_option('db_name')));
